...
当前会读取 README、docs、tests、eval 和 git log，并生成 context summary、project status report、next tasks、risk report、commit suggestions、Tool Call Log 和 Run Log。
...
真实 RAGHub 运行中记录了 9 条 tool call，包括 context_reader、git_reader、project_status_analyzer、readme_advisor、risk_advisor 和 commit_advisor 等。
...
`run_logs/latest_run.json` 包含 workflow_status、steps、tool_calls、human_confirmation_status、files_read、git_commits_read 和 delivery_readiness_score。
...
真实运行读取 24 个文件、10 条 git commit，生成 7 个 outputs 和 1 个 run log。
...
## Q：Delivery Readiness Score 为什么不是生产级评分？

### 回答要点

当前分数是规则化证据完整度检查，只说明求职展示范围内的 README、docs、tests、eval、bad cases、问题复盘和 git 记录是否较完整。

### 项目中的具体实现

CLI 和报告中都会说明“该分数表示目标项目在当前展示范围内证据较完整，不代表生产级可用”。
...
最大边界是当前不接真实 LLM，分析深度主要来自规则和读取到的显式证据，无法做复杂语义判断。
